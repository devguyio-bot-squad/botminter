...
description: >-
  ...
  Scans the project board for po:* statuses and dispatches work
  to specialized product ownership hats via priority table.
...
# Board Scanner (Human-Assistant Scope)

This skill defines your PLAN step when coordinating. Scan the GitHub
Projects v2 board for `po:*` statuses, then DELEGATE by publishing
...
Use the results to identify items with `po:*` statuses and their issue numbers
for dispatch.
...
2026-03-02T10:15:01Z — board.scan — 2 po issues found
...
```
### 📝 po — $(date -u +%Y-%m-%dT%H:%M:%SZ)

Auto-advance: po:merge → done
```
...
Dispatch based on the highest-priority `po:*` status found. Process one
...
| 5 | `po:backlog` | `po.backlog` |
| 6 | `po:ready` | `po.backlog` |

No po work found → emit `LOOP_COMPLETE`.
...
are dispatched to `review_gater` each scan cycle. The `review_gater` hat checks
for a human response, acts if found, and returns control if not. This
