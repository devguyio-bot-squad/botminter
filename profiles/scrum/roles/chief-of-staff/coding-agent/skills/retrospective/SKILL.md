...
```bash
# List completed issues in the milestone
gh issue list --repo "$TEAM_REPO" --milestone "<milestone>" --state closed --json number,title,labels,closedAt,comments

# Or by date range
gh issue list --repo "$TEAM_REPO" --state closed --json number,title,labels,closedAt,comments | \
  jq '[.[] | select(.closedAt >= "<start-date>")]'
```
