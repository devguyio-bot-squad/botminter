...
use crate::config;
use crate::daemon::{ForceStopResponse, RetriggerFinalizationResponse, SessionsListResponse, StopSessionResponse};
use crate::formation;
use crate::team::Team;
...
mod session_cli_stop_tests {
    ...
    fn stop_member_triggers_graceful_deactivation_for_all_member_sessions() {
        ...
        let list_fn = || Ok(SessionsListResponse { sessions: sessions.clone() });
        let stop_fn = |id: &str| {
            stopped_ids.borrow_mut().push(id.to_string());
            Ok(StopSessionResponse { ok: true, dirty_repos: vec![], error: None })
        };

        let summary = stop_member_sessions("alice", &list_fn, &stop_fn).unwrap();

        let ids = stopped_ids.borrow();
        assert!(ids.contains(&"s1".to_string()), "alice's loop session must be stopped; stopped: {ids:?}");
        assert!(ids.contains(&"s2".to_string()), "alice's brain session must be stopped; stopped: {ids:?}");
        assert!(!ids.contains(&"s3".to_string()), "bob's session must NOT be stopped; stopped: {ids:?}");
        assert_eq!(summary.sessions_deactivating, 2, "2 alice sessions must be deactivating");
    }
    ...
    fn stop_session_specific_only_stops_that_session() {
        ...
        let ids = stopped_ids.borrow();
        assert_eq!(ids.len(), 1, "exactly one session must be stopped; stopped: {ids:?}");
        assert_eq!(ids[0], "target-session", "only the target session must be stopped");
        assert_eq!(summary.sessions_deactivating, 1, "1 session must be deactivating");
    }
    ...
    fn bare_stop_stops_autonomous_skips_interactive() {
        ...
        let ids = stopped_ids.borrow();
        assert!(ids.contains(&"s1".to_string()), "loop session must be stopped; stopped: {ids:?}");
        assert!(ids.contains(&"s2".to_string()), "brain session must be stopped; stopped: {ids:?}");
        assert!(!ids.contains(&"s3".to_string()), "interactive session must NOT be stopped; stopped: {ids:?}");
        assert_eq!(summary.interactive_skipped, 1, "1 interactive session must be reported as skipped");
        assert_eq!(summary.sessions_deactivating, 2, "2 autonomous sessions must be deactivating");
    }
    ...
    fn force_stop_active_session_kills_immediately_no_finalization() {
        let force_fn = |id: &str| Ok(ForceStopResponse {
            ok: true,
            session_id: id.to_string(),
        ...
        assert_eq!(summary.sessions_deactivating, 1, "1 session must be deactivating");
        assert_eq!(summary.entering_finalization, 0, "force stop must NOT launch finalization");
    }
    ...
    fn force_stop_finalizing_session_terminates_subagent_retained() {
        ...
        assert_eq!(summary.sessions_deactivating, 1, "1 session must be deactivating");
        assert_eq!(summary.entering_finalization, 0, "force stop on Finalizing must not launch new finalization");
    }
    ...
    fn retrigger_after_force_stop_launches_fresh_finalization() {
        ...
        assert!(*retrigger_called.borrow(), "retrigger function must be called");
    }
    ...
    fn stop_returns_immediately_with_summary() {
        ...
        let list_fn = || Ok(SessionsListResponse { sessions: sessions.clone() });
        let stop_fn = |_id: &str| Ok(StopSessionResponse {
            ok: true,
        ...
        assert_eq!(summary.sessions_deactivating, 2, "both sessions must be deactivating");
        assert_eq!(summary.entering_finalization, 2, "both dirty sessions must be entering finalization");
