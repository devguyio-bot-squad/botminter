...
impl Lima {
        ...
        if which::which("limactl").is_err() {
            bail!(
                "limactl is not installed.\n\n\
                 ...
                 \n\
                 Then run `bm teams bootstrap` again."
            );
        }
    ...
    pub fn bootstrap(
        ...
        let template = generate_template(vm_name, cpus, memory, disk, mount_path);
...
pub fn generate_template(vm_name: &str, cpus: u32, memory: &str, disk: &str, mount_path: &str) -> String {
    ...
    format!(
        r#"# Lima template generated by `bm teams bootstrap` for VM "{vm_name}"
...
mod tests {
    ...
    fn generate_template_contains_required_tools() {
        let template = generate_template("test-vm", 4, "8GiB", "100GiB", "~/.botminter");
    ...
    fn generate_template_custom_resources() {
        let template = generate_template("custom", 8, "16GiB", "200GiB", "~/.botminter");
    ...
    fn generate_template_has_readiness_probe() {
        let template = generate_template("probe-vm", 4, "8GiB", "100GiB", "~/.botminter");
    ...
    fn generate_template_is_idempotent() {
        let template = generate_template("idemp-vm", 4, "8GiB", "100GiB", "~/.botminter");
    ...
    #[test]
    fn generate_template_mounts_botminter_dir_only() {
        let template = generate_template("mount-vm", 4, "8GiB", "100GiB", "~/.botminter");
        assert!(template.contains("~/.botminter"));
        assert!(template.contains("writable: true"));
    }

    #[test]
    fn generate_template_embeds_vm_name() {
        let template = generate_template("my-team", 4, "8GiB", "100GiB", "~/.botminter");
        assert!(template.contains(r#"for VM "my-team""#));
    }

    #[test]
    fn generate_template_is_valid_yaml() {
        let template = generate_template("yaml-check", 4, "8GiB", "100GiB", "~/.botminter");
    ...
    fn generate_template_user_mode_provision() {
        let template = generate_template("mode-vm", 4, "8GiB", "100GiB", "~/.botminter");
    ...
    fn generate_template_selinux_comment() {
        let template = generate_template("sel-vm", 4, "8GiB", "100GiB", "~/.botminter");
