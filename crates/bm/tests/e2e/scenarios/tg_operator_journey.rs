...
use super::super::helpers::{
    cleanup_project_boards, read_pid_from_state, E2eConfig, GithubSuite, ProcessGuard,
};
    ...
    move |env| {
        let stdout = env
            ...
            .env(
                &format!(
                    "BM_BRIDGE_TOKEN_{}",
                    MEMBER_DIR.to_uppercase().replace('-', "_")
                ),
                BOT_TOKEN,
            )
            .run();
...
fn build_suite(gh_org: String, gh_token: String, config: &E2eConfig) -> GithubSuite {
    ...
    GithubSuite::new_self_managed("scenario_tg_operator_journey", &repo_full_name)
        ...
        .case(
            "01_init_with_tg_bridge",
            init_with_tg_bridge_fn(gh_org.clone(), gh_token.clone()),
        )
        .case(
            "02_hire_member",
            hire_member_fn(
            ...
            move |env: &mut TestEnv| {
                ...
                if let Some(cid) = env.get_export("tg_mock_container_id") {
                    let _ = env
                        ...
                        .args(["stop", "-t", "2", cid])
                        .output();
                    let _ = env.command("podman").args(["rm", "-f", cid]).output();
                }
