...
#
# deploy.sh — Deploy synthetic fixtures into a compact-profile team repo for M2 Sprint 5 validation.
#
# Usage: bash deploy.sh <team-repo-path> [--project-repo=<path>]
#
# Prerequisites: team repo must be initialized and superman member added:
#   just init --repo=<path> --profile=compact project=hypershift
#   cd <path> && just add-member superman
#
# This script:
#   1. Copies knowledge files (3 scopes) into the team repo — member scope uses team/superman/
#   2. Copies invariant files (3 scopes) into the team repo — member scope uses team/superman/
...
#
# Fixture files are sourced from the shared M2 fixtures directory (../../fixtures/).
# Only the deployment paths differ: compact profile has a single "superman" member
# instead of separate "architect" and "human-assistant" members.
#
# Idempotent: re-running produces no changes if already deployed.

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SHARED_FIXTURES="$SCRIPT_DIR/../../fixtures"
TEAM_REPO=""
...
# Verify shared fixtures exist
if [ ! -d "$SHARED_FIXTURES" ]; then
    echo "Error: shared fixtures not found at $SHARED_FIXTURES"
    exit 1
fi

# Verify superman member has been added (prerequisite)
if [ ! -d "$TEAM_REPO/team/superman" ]; then
    echo "Error: superman member not found at $TEAM_REPO/team/superman/"
    echo "Run 'just add-member superman' in the team repo first."
    exit 1
fi

echo "Deploying synthetic fixtures (compact profile) to $TEAM_REPO"

# --- 1. Knowledge files (3 scopes) ---

# Team-level knowledge
mkdir -p "$TEAM_REPO/knowledge"
cp "$SHARED_FIXTURES/knowledge/commit-convention.md" "$TEAM_REPO/knowledge/commit-convention.md"
echo "  [ok] Team knowledge: knowledge/commit-convention.md"

# Project-level knowledge
mkdir -p "$TEAM_REPO/projects/hypershift/knowledge"
cp "$SHARED_FIXTURES/projects/hypershift/knowledge/hcp-architecture.md" \
...
# Member-level knowledge — superman (not architect)
mkdir -p "$TEAM_REPO/team/superman/knowledge"
cp "$SHARED_FIXTURES/team/architect/knowledge/design-patterns.md" \
   "$TEAM_REPO/team/superman/knowledge/design-patterns.md"
echo "  [ok] Member knowledge: team/superman/knowledge/design-patterns.md"

# --- 2. Invariant files (3 scopes) ---

# Team-level invariant
mkdir -p "$TEAM_REPO/invariants"
cp "$SHARED_FIXTURES/invariants/code-review-required.md" "$TEAM_REPO/invariants/code-review-required.md"
...
cp "$SHARED_FIXTURES/projects/hypershift/invariants/upgrade-path-tests.md" \
...
# Member-level invariant — superman (not architect)
mkdir -p "$TEAM_REPO/team/superman/invariants"
cp "$SHARED_FIXTURES/team/architect/invariants/design-quality.md" \
   "$TEAM_REPO/team/superman/invariants/design-quality.md"
echo "  [ok] Member invariant: team/superman/invariants/design-quality.md"

# --- 3. Synthetic epic ---

mkdir -p "$TEAM_REPO/.github-sim/issues"
cp "$SHARED_FIXTURES/synthetic-epic-1.md" "$TEAM_REPO/.github-sim/issues/1.md"
...
Synthetic HCP project repository for M2 Sprint 5 validation.
...
if git diff --quiet && git diff --cached --quiet && [ -z "$(git ls-files --others --exclude-standard)" ]; then
    ...
    git add -A
    git commit --quiet -m "feat(fixtures): deploy synthetic fixtures for M2 Sprint 5 validation

Adds knowledge (3 scopes), invariants (3 scopes), and synthetic epic
at status/po:triage for single-agent compact profile lifecycle validation.

Ref: M2-Sprint-5-Step-5"
    echo ""
...
echo "  1. Create workspace:        cd $TEAM_REPO && just create-workspace superman $PROJECT_REPO"
echo "  2. Launch agent:            cd $TEAM_REPO && just launch superman --telegram-bot-token <token>"
